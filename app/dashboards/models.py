...
class Laptops(models.Model):
    ...
    price = models.TextField(db_collation='SQL_Latin1_General_CP1_CI_AS', blank=True)
    ram_gb_field = models.IntegerField(db_column='ram (GB)', blank=True)
    warranty_meses_field = models.FloatField(db_column='warranty (meses)', blank=True)
    screensize_pulgadas_field = models.FloatField(db_column='screenSize (Pulgadas)', blank=True)
...
class Dashboard(TokenBasedModel):
    ...
    last_modified_by = models.ForeignKey('user.User', on_delete=models.CASCADE, related_name='dashboards', null=True, blank=True)
